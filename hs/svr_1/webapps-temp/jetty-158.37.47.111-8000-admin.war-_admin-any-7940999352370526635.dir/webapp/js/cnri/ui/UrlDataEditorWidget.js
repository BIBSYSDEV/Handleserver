function UrlDataEditorWidget(containerDiv, handleValue) {
	var self = this;
	...
	function constructor() {
	    widgetId = UrlDataEditorWidget.count++;
		var form = $('<form class="form-horizontal" role="form"></form>');
		form.submit(function(e) {return false;}); 
		containerDiv.append(form);
		urlInput = self.addTextInput(form, "URL", "inputUrl-"+widgetId, handleValue.data.value, onUrlInputValueChange);
	}
	...
	function onUrlInputValueChange(e) {
        if (isObject(handleValue.data.value) || isArray(handleValue.data.value)) {
            handleValue.data.value = JSON.parse(urlInput.val());
        } else {
            handleValue.data.value = urlInput.val();
        }
	}
