...
function DSASign(Hm, K) {
    if (K == undefined) {
        K = DSAGenerateK.call(this);
    } else if (K.compareTo(BigInteger.ZERO) <= 0 ||
        this.q.compareTo(K) <= 0) {
        throw 'K is not between 1 and q-1';
    }
    var r = this.g.modPow(K, this.p).mod(this.q);
    var k1 = K.modInverse(this.q);
    var s = k1.multiply(Hm.add(this.x.multiply(r))).mod(this.q);
    if (r.equals(BigInteger.ZERO) || s.equals(BigInteger.ZERO)) return DSASign.call(this,Hm);
    return [r,s];
}

function DSAGenerateK() {
    var rng = new SecureRandom();
    while (true) {
        var K = new BigInteger(this.q.bitLength(), rng);
        if (K.compareTo(BigInteger.ZERO) > 0 && this.q.compareTo(K) > 0) return K;
    }
}
