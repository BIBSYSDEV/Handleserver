...
var secretKeyAuthInfo = cnri.util.TestUtil.secretKeyAuthInfo;
var testHandleString = cnri.util.TestUtil.TEST_PREFIX+"/qunit-1"; 

test("Secret key auth create and resolve", function() {
    stop(); //Tells the qUnit framework to wait until it gets the start command
    init();
});

function init() {
    client.setAuthInfo(secretKeyAuthInfo);
...
function createHandle() {
    var handleRecord = cnri.util.TestUtil.createHandleJavaScriptObject(secretKeyAuthInfo.id, testHandleString);
